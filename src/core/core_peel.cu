...
__global__ auto scan_kernel_adv(int* d_degree, uint* d_peels, uint* d_peel_idx,
                                 int alpha, int beta, uint u_num, uint num_vertex) -> void {
    ...
    for (uint base = 0; base < num_vertex; base += N_THREADS) {
        ...
        if (d_degree[v] < threshold) {
            uint idx = atomicAdd(d_peel_idx, 1);
            d_peels[idx] = v;
        }
    }
...
__global__ auto peel_kernel_adv(const uint* d_offset, const uint* d_neighbors, int* d_degree,
                                 uint u_num, int alpha, int beta, uint* d_peels, uint* d_idx, uint* d_next,
                                uint* d_next_idx) -> void {


    uint warp_id = threadIdx.x / 32;
    uint lane_id = threadIdx.x % 32;
    uint global_warp_id = blockIdx.x * (blockDim.x / 32) + warp_id;
    uint num_warps = gridDim.x * (blockDim.x / 32);

    for (uint i = global_warp_id; i < *d_idx; i += num_warps) {
        uint v = d_peels[i];
        uint start = d_offset[v];
        uint end = d_offset[v + 1];


        for (uint j = start + lane_id; j < end; j += 32) {
            uint u = d_neighbors[j];
            int threshold = u < u_num ? alpha : beta;
        ...
        // wait for all threads to finish in the same warp
        __syncwarp();
    }
}
...
auto g_abcore_peeling_adv(Graph& g, int alpha, int beta) -> double {
    ...
    CER(cudaMalloc(&d_neighbors, sizeof(uint) * g.m * 2));
    CER(cudaMalloc(&d_degree, sizeof(int) * g.n));
    CER(cudaMalloc(&d_peels, sizeof(uint) * g.n));
    CER(cudaMalloc(&d_peel_idx, sizeof(uint)));
    CER(cudaMalloc(&d_next, sizeof(uint) * g.n));
    CER(cudaMalloc(&d_next_idx, sizeof(uint)));

    CER(cudaMemcpy((void*) d_offset, (void*) g.offsets, sizeof(uint) * (g.n + 1), cudaMemcpyHostToDevice));
    CER(cudaMemcpy((void*) d_neighbors, (void*) g.neighbors, sizeof(uint) * g.m * 2, cudaMemcpyHostToDevice));
    CER(cudaMemcpy((void*) d_degree, (void*) g.degrees, sizeof(uint) * g.n, cudaMemcpyHostToDevice));
    CER(cudaMemset((void*) d_peel_idx, 0, sizeof(uint)));
    CER(cudaMemset((void*) d_next_idx, 0, sizeof(uint)));
    ...
    scan_kernel_adv<<<BLK_NUMS, BLK_DIM, sizeof(uint) * BLK_DIM>>>(d_degree, d_peels, d_peel_idx, alpha, beta, g.u_num, g.n);
    CER(cudaMemcpy(&cnt, d_peel_idx, sizeof(uint), cudaMemcpyDeviceToHost));

    while (cnt != 0) {
        peel_kernel_adv<<<BLK_NUMS, BLK_DIM>>>(d_offset, d_neighbors, d_degree, g.u_num, alpha, beta, d_peels, d_peel_idx, d_next, d_next_idx);
        CER(cudaMemcpy(d_peel_idx, d_next_idx, sizeof(uint), cudaMemcpyDeviceToDevice));
        CER(cudaMemcpy(d_peels, d_next, sizeof(uint) * g.n, cudaMemcpyDeviceToDevice));
        CER(cudaMemcpy(&cnt, d_peel_idx, sizeof(uint), cudaMemcpyDeviceToHost));
    }
    cudaDeviceSynchronize();
